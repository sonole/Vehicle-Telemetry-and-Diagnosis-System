...
window.air = function () {
  var dpMin, dpMax;
  dpMin = new air_datepicker__WEBPACK_IMPORTED_MODULE_1__["default"]('#datestart', {
    locale: customLocale,
    buttons: ['clear'],
    timepicker: true,
  ...
  dpMax = new air_datepicker__WEBPACK_IMPORTED_MODULE_1__["default"]('#datefinish', {
    locale: customLocale,
    buttons: ['clear'],
    timepicker: true,
...
//Draw the $path var
window.draw = /*#__PURE__*/function () {
  var _ref3 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(ms) {
    var drawnPath, i, _i, sleep;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 8:
            map.drawPolyline({
              path: [[drawnPath[_i][0], drawnPath[_i][1]], [drawnPath[_i + 1][0], drawnPath[_i + 1][1]]],
              strokeColor: '#ff0000',
              strokeOpacity: 0.8,
              strokeWeight: 4
            });
...
window.addCustomMarker = function (coordinates, title, content, image) {
...
window.tableWithPagination = function () {
  // Select all the pagination buttons
  var paginateButtons = document.querySelectorAll('button[data-page]');
  ...
  paginateButtons.forEach(function (button) {
    button.addEventListener('click', function () {
      ...
      var data = paginateArray(path, 10, pageNumber);

      // For each pagination number make the thead only 1 time
      // For each pagination number make the tbody
      document.getElementById("tablebody").innerHTML = '';
      var thead = [];
      var elem = document.querySelector("#tablehead");
      var isHeadCreated = elem.childNodes.length;
      var _loop = function _loop(_i2) {
        var cols = '';
        Object.keys(data[_i2]['general']).forEach(function (key) {
          if (key !== 'sn') {
            if (isHeadCreated === 0) {
              thead.push(key);
            }
            cols += '<td class="p-1.5">' + data[_i2]['general'][key] + '</td>';
          }
        });
        Object.keys(data[_i2]['GPS']).forEach(function (key) {
          if (key !== 'lat' && key !== 'lng') {
            if (isHeadCreated === 0) {
              thead.push(key);
            }
            cols += '<td class="p-1.5">' + data[_i2]['GPS'][key] + '</td>';
          }
        });
        Object.keys(data[_i2]['OBD']).forEach(function (key) {
          if (isHeadCreated === 0) {
            thead.push(key);
          }
          cols += '<td class="p-1.5">' + data[_i2]['OBD'][key] + '</td>';
        });
        if (isHeadCreated === 0) {
          var thRow = '<tr class="border-b border-emerald-700">';
          thead.forEach(function (key) {
        ...
        var row = '<tr onclick="rowClick(' + data[_i2]['GPS']['lat'] + ',' + data[_i2]['GPS']['lng'] + ',' + '\'' + data[_i2]['general']['Date & Time'] + '\'' + ')" ' + 'onmouseout="rowHoverOut(this)" onmouseover="rowHoverIn(this)" class="text-emerald-500 text-md text-center">' + cols + ' </tr>';
 ...
 * Helpers
*/
window.rowClick = function (lat, lng, dt) {
  addCustomMarker([lat, lng], dt);
};
window.rowHoverIn = function (e) {
  e.style.cursor = 'pointer';
  e.style.color = '#01579b';
};
window.rowHoverOut = function (e) {
  e.style.cursor = 'auto';
...
window.findClosestPair = function (clickedCoordinates) {
  var min = getDistance(clickedCoordinates[0], clickedCoordinates[1], path[0]['GPS']['lat'], path[0]['GPS']['lng']);
  var minIndex = 0;
  for (var i = 1; i <= packetsNo - 1; i++) {
    var temp = getDistance(clickedCoordinates[0], clickedCoordinates[1], path[i]['GPS']['lat'], path[i]['GPS']['lng']);
    if (temp < min) {
      min = temp;
      minIndex = i;
    }
